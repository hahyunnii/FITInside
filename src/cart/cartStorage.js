...
export const removeFromCart = async (id) => {
    ...
    await updateLocalCart(updatedCart);
};
...
export const fetchAndMergeCartData = async (token) => {
    try {
        ...
        if (response.ok) {
            const result = await response.json();
            if (Array.isArray(result.carts)) {
                const dbCart = result.carts.map(item => ({
                    id: item.productId,
                    quantity: item.quantity
                }));
                localStorage.setItem(DB_CART_KEY, JSON.stringify(dbCart));
                return mergeCartData(dbCart);
            }
            throw new Error('장바구니 데이터 형식 오류');
        }
        throw new Error(`API 응답 오류: ${response.status}`);
...
// 장바구니 데이터 병합
export const mergeCartData = async (fetchedCartData) => {
    const existingCartData = getCart();
    const mergedData = fetchedCartData.map(fetchedItem => {
        const existingItem = existingCartData.find(item => item.id === fetchedItem.productId);
        return existingItem ? { ...existingItem, quantity: fetchedItem.quantity } : { id: fetchedItem.productId, quantity: fetchedItem.quantity };
    });

    const finalMergedData = [...mergedData, ...existingCartData.filter(existingItem => !fetchedCartData.some(fetchedItem => fetchedItem.productId === existingItem.id))];
    await updateLocalCart(finalMergedData);
...
const updateDBWithDifferences = async (localCart) => {
    ...
    const dbCart = JSON.parse(localStorage.getItem(DB_CART_KEY)) || [];
    ...
    for (const dbItem of dbCart) {
        if (!localCart.some(item => item.id === dbItem.id)) {
            await deleteCartItem(dbItem);
        }
    }

    localStorage.setItem(DB_CART_KEY, JSON.stringify(localCart));
};

// DB에 장바구니 아이템 추가
const postCartItem = async (item) => {
    const requestData = { productId: item.id, quantity: item.quantity };
    await fetch('http://localhost:8080/api/carts', {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
